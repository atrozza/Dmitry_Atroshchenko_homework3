...
def pairs(numbers_string):
    ...
    return total_pairs
    # Решение верно и проходит тесты, не самая очевидная формула подсчета общего количества пар. Для альтернативного решения - см ответы
...
# Решение вполне ок, тесты проходит. Есть решение намного проще - см ответы
...
# Интересное решение)) Но тесты проходит - окей. Для более пайтоник - см ответы
...
def euclid(a,b):
    ...
    return a
    # Все верно - самый ок вариант решения
...
def cities(input_string):
    ...
    for i in range(1, n + 1):
        ...
        for city in cities:
            # Все ок, тут можно попробовать сократить строки через dict.get()
    ...
    return '\n'.join(result)
# Все супер - отличное лаконичное решение.
...
def languages(input_string):
    ...
    return '\n'.join(result)
# Тоже вполне хорошее решение, круто, что применено много методов с занятий. Для др решений - см ответы.
